...
class finetuned_RibonanzaNet(RibonanzaNet):
    def __init__(self, rnet_config, config, pretrained=False):
        ...
        self.time_norm1=nn.LayerNorm(decoder_dim)

        self.time_mlp2=nn.Sequential(
                                     nn.Linear(decoder_dim,decoder_dim*2),
                                     nn.ReLU(),  
                                     nn.Linear(decoder_dim*2,decoder_dim*1))
        self.time_norm2=nn.LayerNorm(decoder_dim)

        self.time_mlp3=nn.Sequential(
                                     nn.Linear(decoder_dim,decoder_dim*2),
                                     nn.ReLU(),  
                                     nn.Linear(decoder_dim*2,decoder_dim*1))
        self.time_norm3=nn.LayerNorm(decoder_dim)

        self.tgt_norm=nn.LayerNorm(decoder_dim)
    ...
    def get_decoder_features(self, sequence_features, pairwise_features, xyz, t):
        ...
        distance_features= checkpoint.checkpoint(self.custom(self.embed_pair_distance), [pairwise_features,xyz],use_reentrant=False)
        ...
        tgt=self.tgt_norm(time_embed+self.adaptor(sequence_features)+xyz)
    ...
    def forward(self,src,xyz,t, N_cycle=4):
        ...
        tgt, pairwise_features, distance_features=self.get_decoder_features(sequence_features, pairwise_features, xyz, t)
        ...
        for layer in self.structure_module:
            ...
            [tgt, sequence_features,pairwise_features,xyz,distance_features,None],
            use_reentrant=False)
    ...
    def denoise(self,sequence_features,pairwise_features,xyz,t):
        ...
        for layer in self.structure_module:
            tgt=layer([tgt, sequence_features,pairwise_features,xyz,distance_features,None])
            # xyz=xyz+self.xyz_predictor(sequence_features).squeeze(0)
            # xyzs.append(xyz)
            #print(sequence_features.shape)
        xyz=self.xyz_predictor(tgt).squeeze(0)
    ...
    def sample_euler(self, src, N, num_steps=None, eta=0.0, N_cycle=4):
        ...
        for i in range(len(timesteps)):
            ...
            #sigma_t = eta * torch.sqrt((1 - alpha_prev) / (1 - alpha_t) * (1 - alpha_t/alpha_prev))
            sigma_t = eta * torch.sqrt((1 - alpha_bar_next) / (1 - alpha_bar_t) * (1 - alpha_bar_t / alpha_bar_next))
            x_t = x_t * scale_factor - eps1 * step_size + torch.randn_like(x_t) * sigma_t
