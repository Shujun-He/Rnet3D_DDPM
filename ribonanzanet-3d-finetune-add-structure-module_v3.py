...
class finetuned_RibonanzaNet(RibonanzaNet):
    def __init__(self, config, pretrained=False):
        ...
        self.structure_module=[SimpleStructureModule(d_model=768, nhead=12, 
                 dim_feedforward=768*4, pairwise_dimension=64, dropout=0.2) for i in range(6)]
        self.structure_module=nn.ModuleList(self.structure_module)

        self.xyz_embedder=nn.Linear(3,768)
        self.xyz_norm=nn.LayerNorm(768)
        self.xyz_predictor=nn.Linear(768,3)
        
        self.adaptor=nn.Sequential(nn.Linear(256,768),nn.LayerNorm(768))
...
best_loss=np.inf
optimizer = torch.optim.Adam(model.parameters(), weight_decay=0.0001, lr=0.0001) #no weight decay following AF
...
for epoch in range(epochs):
    ...
    for idx, batch in enumerate(tbar):
        ...
        dmse_losses=[]
        for i in range(16):
            dmse_losses.append(local_dRMSD(pred_xyz[i],pred_xyz[i],aug_xyz[:,2:][i],aug_xyz[:,2:][i]))
        dmse_losses=torch.stack(dmse_losses,0).mean()

        loss=loss+dmse_losses
        # exit()
        # loss=0
