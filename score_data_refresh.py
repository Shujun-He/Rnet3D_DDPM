...
submission=pd.read_csv("recycle_256.yaml_10_cycles_200_steps_0.0_eta.csv")
id_map=pd.read_csv("../CONFIDENTIAL/rerun_DATA_REFRESH/id_map.csv")
id_map={str(k):str(v) for k,v in zip(id_map['orig'],id_map['new'])}

submission['pdb_id']=submission["ID"].apply(lambda x: x.split("_")[0]).apply(lambda x: id_map[x])
submission['ID']=submission['pdb_id'] + "_" + submission['resid'].astype(str)

public=solution[solution['Usage']=='Public'].reset_index(drop=True)
private=solution[solution['Usage']=='Private'].reset_index(drop=True)

scores,results_per_sub,outputs, pdb_ids=score(public,submission,'ID')
print("public mean")
print(np.mean(scores))

scores,results_per_sub,outputs, pdb_ids=score(private,submission,'ID')
print("private mean")
print(np.mean(scores))
