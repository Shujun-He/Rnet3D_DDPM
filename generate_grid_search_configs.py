...
# === Paths ===
BASE_CONFIG_PATH = "stage1_singlenode.yaml"
...
# Define search grid
beta_scales = [1.0]  # scale both beta_min and beta_max
distogram_weights = [0.03]
data_stds = [35]
grad_clips = [10.0]
learning_rates = [2e-4, 3e-4, 4e-4, 5e-4]
loss_power_scales = [0]
...
combinations = list(itertools.product(beta_scales, distogram_weights, data_stds, grad_clips, learning_rates, loss_power_scales))

for i, (scale, dweight, std, clip, learning_rate, loss_power_scale) in enumerate(combinations):
    ...
    config["loss_power_scale"] = loss_power_scale
